...
describe('AiChat — EventSource messaging (lines 69-88)', () => {
  ...
  it('accumulates token messages via onmessage', async () => {
    ...
    act(() => {
      es.onmessage?.({ data: JSON.stringify({ token: 'Hi ' }) } as MessageEvent);
      es.onmessage?.({ data: JSON.stringify({ token: 'there' }) } as MessageEvent);
    });

    expect(storeState.updateLastAssistantMessage).toHaveBeenLastCalledWith('Hi there');
  });

  it('[DONE] closes EventSource and clears loading (line 69)', async () => {
    render(<AiChat />);
    const textarea = screen.getByPlaceholderText(/Ask your knowledge base/i);
    ...
    // After [DONE] button should re-enable (loading=false)
    await waitFor(() =>
  ...
  it('appends error token on error payload (lines 78-81)', async () => {
    render(<AiChat />);
    const textarea = screen.getByPlaceholderText(/Ask your knowledge base/i);
    fireEvent.change(textarea, { target: { value: 'err test' } });
    fireEvent.click(screen.getByRole('button', { name: /Send/i }));

    await waitFor(() => expect(getLatestES()).toBeTruthy());
    act(() => {
      getLatestES().onmessage?.({ data: JSON.stringify({ error: 'oops' }) } as MessageEvent);
    });
    expect(storeState.updateLastAssistantMessage).toHaveBeenCalledWith(
...
describe('AiChat — EventSource onerror (lines 91-96)', () => {
  ...
  it('calls updateLastAssistantMessage with connection error on onerror', async () => {
    render(<AiChat />);
    const textarea = screen.getByPlaceholderText(/Ask your knowledge base/i);
    fireEvent.change(textarea, { target: { value: 'onerror test' } });
    fireEvent.click(screen.getByRole('button', { name: /Send/i }));
