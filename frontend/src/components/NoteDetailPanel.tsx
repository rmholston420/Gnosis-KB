  ...
  const runAction = useCallback(
    async (type: ActionResult['type']) => {
      ...
      try {
        let content = '';
        if (type === 'summary') {
          const res = (await api.summarizeNote(note.id)) as unknown as { summary: string };
          content = res.summary;
        } else if (type === 'critique') {
          const res = (await api.critiqueNote(note.id)) as unknown as {
            overall?: string;
            strengths?: string[];
            weaknesses?: string[];
            suggestions?: string[];
          ...
          const res = (await api.suggestLinks(note.id)) as unknown as { suggestions: Array<{ title: string; reason: string }> };
