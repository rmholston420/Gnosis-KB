...
def upgrade() -> None:
    ...
    if dialect == "postgresql":
        conn.execute(sa.text("ALTER TABLE notes ADD COLUMN IF NOT EXISTS search_vector tsvector"))
        conn.execute(
            sa.text(
        ...
        if not _column_exists(conn, "notes", "search_vector"):
            conn.execute(sa.text("ALTER TABLE notes ADD COLUMN search_vector TEXT"))
...
def downgrade() -> None:
    ...
    dialect = conn.dialect.name
    if dialect == "postgresql":
        conn.execute(sa.text("DROP TRIGGER IF EXISTS notes_search_vector_trigger ON notes"))
        conn.execute(sa.text("DROP FUNCTION IF EXISTS notes_search_vector_update"))
        conn.execute(sa.text("DROP INDEX IF EXISTS ix_notes_search_vector"))
    # SQLite: DROP COLUMN requires 3.35+; skip silently on older versions
    try:
        conn.execute(sa.text("ALTER TABLE notes DROP COLUMN IF EXISTS search_vector"))
    except Exception:
        pass
