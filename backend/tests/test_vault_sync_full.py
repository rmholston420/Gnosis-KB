...
async def test_run_full_sync_no_vault_path():
    ...
    with (
        patch("gnosis.services.vault_sync.get_settings") as mock_cfg,
        patch(
            "gnosis.services.vault_sync._resolve_owner_id", new_callable=AsyncMock, return_value=1
        ),
...
async def test_run_full_sync_empty_vault(tmp_path: Path):
    ...
    try:
        with (
            patch("gnosis.services.vault_sync.get_settings") as mock_cfg,
            patch(
                "gnosis.services.vault_sync._resolve_owner_id",
