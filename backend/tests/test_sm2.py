...
Moved here from backend/gnosis/core/sm2_test.py — test files belong in
backend/tests/, not co-located inside the source package.

Run with:  pytest backend/tests/test_sm2.py -v
"""
...
def test_incorrect_resets_repetitions():
    ...
    assert new_state.interval == 1
    assert new_due == date.today() + timedelta(days=1)
    # Easiness is unchanged on an incorrect answer
