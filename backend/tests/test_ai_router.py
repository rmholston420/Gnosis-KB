...
async def test_get_providers_available(client):
    ...
    with patch(_LLM_PATCH, mock_llm), patch("gnosis.routers.ai.httpx.AsyncClient") as mock_http:
        mock_resp = MagicMock()
...
async def test_chat_uses_qdrant_rag_when_graph_unavailable(client):
    with (
        patch(_GRAPH_PATCH, _mock_graph(available=False)),
...
async def test_chat_uses_graph_rag_when_available(client):
    with (
        patch(_GRAPH_PATCH, _mock_graph(available=True, answer="graph answer")),
...
async def test_chat_503_when_no_provider(client):
    mock_llm = MagicMock()
    mock_llm.is_available = False
    with patch(_GRAPH_PATCH, _mock_graph(available=False)), patch(_LLM_PATCH, mock_llm):
        r = await client.post("/api/v1/ai/chat", json={"message": "question"})
...
async def test_stream_chat_graph_rag_path(client):
    ...
    with patch(_GRAPH_PATCH, _mock_graph(available=True)), patch(_LLM_PATCH, _mock_llm()):
        r = await client.get("/api/v1/ai/stream/chat?message=hello")
    assert r.status_code == 200
